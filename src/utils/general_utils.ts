...
export function findObj(theObj, theDoc = null) {
    let p, i, foundObj;
...
export function stripUnwanted(pNode) {
    let vAllowedTags = ['#text', 'p', 'br', 'ul', 'ol', 'li', 'div', 'span', 'img'];
    for (let i = 0; i < pNode.childNodes.length; i++) {
...
export function delayedHide(pGanttChartObj, pTool, pTimer) {
...
export function benchMark(pItem) {
...
export function calculateCurrentDateOffset(curTaskStart, curTaskEnd) {
...
export function getOffset(pStartDate, pEndDate, pColWidth, pFormat, pShowWeekends) {
...
export function fadeToolTip(pDirection, pTool, pMaxAlpha) {
...
export function updateFlyingObj(e, pGanttChartObj, pTimer) {
    let vCurTopBuf = 3;
    let vCurLeftBuf = 5;
    let vCurBotBuf = 3;
    let vCurRightBuf = 15;
    let vMouseX = (e) ? e.clientX : (window.event as MouseEvent).clientX;
    let vMouseY = (e) ? e.clientY : (window.event as MouseEvent).clientY;
...
export function moveToolTip(pNewX, pNewY, pTool, timer) {
...
export function makeRequest(pFile, json = true, vDebug = false) {
    if ((<any>window).fetch) {
        const f = fetch(pFile);
        if (json) {
...
export function makeRequestOldBrowsers(pFile, vDebug = false) {
